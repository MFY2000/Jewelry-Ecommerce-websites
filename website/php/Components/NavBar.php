<?php


    function navigation($root){
    return "<!-- header start -->
    <nav class='navbar'>
        <div class='container-fluid'>
            <!-- Brand and toggle get grouped for better mobile display -->
            <div class='navbar-header'>
                <button type='button' class='navbar-toggle collapsed' data-toggle='collapse' data-target='#bs-example-navbar-collapse-1' aria-expanded='false'>
                        <span class='sr-only'>Toggle navigation</span>
                        <span class='icon-bar'></span>
                        <span class='icon-bar'></span>
                        <span class='icon-bar'></span>
                    </button>
                <a class='navbar-brand' href='$root/index.php'>
                    <img src='$root/images/Logo/Logo_small.png' alt='logo' />
                </a>
            </div>

            <!-- Collect the nav links, forms, and other content for toggling -->
            <div class='collapse navbar-collapse' id='bs-example-navbar-collapse-1'>
                <div class='navigation'>
                    <ul class='nav navbar-nav'>
                        <li><a href='$root/index.php'>Home</a></li>
                        <li><a href='$root/Pages/aboutus.php'>About Us</a></li>
                        <li><a href='$root/Pages/shop.php'>Shop</a></li>
                        <li><a href='$root/Pages/contactus.php'>ContactUs</a></li>
                    </ul>
                </div>
                <ul class='nav navbar-nav navbar-right'>
                    <li>
                    
                ".(!isset($_SESSION['Login']['id']) ? 
                    "<a href='$root/Pages/login.php?page=signIN'>
                        <img src='$root/images/Icons/userlogo.png' alt='' />
                    </a>":
                    "<a href='$root/php/Services/Function/logout.php' >
                        <img src='$root/images/Icons/logout.png' alt='' style='width: 20px;' />
                    </a>"
                )."
                        
                    </li>
                    <li>
                    ".(isset($_SESSION['Login']['id']) ? 
                    "
                        <a href='$root/Pages/cart.php'>
                            <img src='$root/images/Icons/cartLogo.png' alt='' />
                        </a>
                    ":""
                    )."
                    </li>
                </ul>
            </div>
            <!-- /.navbar-collapse -->
        </div>
        <!-- /.container-fluid -->
    </nav>
    <!-- header end -->
    ";
    }
?>